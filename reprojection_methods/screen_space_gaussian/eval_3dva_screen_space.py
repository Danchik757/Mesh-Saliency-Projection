...
Method (screen_space_gaussian):
  ...
  3. Sample the gaze density image at each vertex's screen position.
...
OBJ subdir:     3DModels-Simplif-up/<model>.obj
...
# Internal gaze density image resolution. Sigma is specified as a fraction of
# image width so it scales correctly when _IMG_W changes.
_IMG_W = 256
_IMG_H = 144
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate screen_space_gaussian on one 3DVA model (per-vertex)."
    )
...
def _apply_transform_no_recenter(
    ...
    Used for vertices where recentering was already applied once outside the
    per-frame loop, using the vertex bounding-box center.
    """
...
def run_screen_space(
    ...
    # Precompute vertex positions once — apply vertex-bbox recenter if requested.
    # Only the rotation/scale/translate is reapplied per frame via
    # _apply_transform_no_recenter (O(n_verts), no BVH copy).
...
def main() -> None:
    ...
    results = {view: compute_metrics(vert_sal, gt) for view, gt in gt_maps.items()}
