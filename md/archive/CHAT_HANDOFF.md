...
> Последнее обновление: 2026-06-07 (session 19). Last commit: `2d79c1f`.
> Содержит **всё необходимое** для понимания проекта без доступа к истории.

---

## 🔑 Быстрый ответ на 4 вопроса (для нового агента)

### 1. Где лежат новые GT и что они из себя представляют

Речь идёт о **3DVA combined GT** — новом типе GT, созданном из трёх авторских GT-файлов.

**Где:**
```
$VISUAL_ATTENTION_3D_SHAPES_ROOT/CombinedGT/
  bunny_combined_gt.txt          — 20 000 строк, одно float на вершину
  bunny_combined_gt_meta.json    — stats (coverage, GT sums per view)
  A380_combined_gt.txt           — 47 756 строк
  ...
  build_summary.json             — итоговая статистика по всем моделям
```

Путь на локальной машине:
```
/Users/admin/Documents/LAB/SALIENCY_code/GAZE_DATA/datasets/3DVA/CombinedGT/
```

**Что это:** авторский датасет 3DVA содержит 3 GT-файла на каждую из 32 моделей
(суффиксы `_300norm`, `_413norm`, `_599norm`) — три **независимых статичных ракурса**.
Каждый файл = плотность фиксаций 19 участников при взгляде на объект с одной стороны.

Проблема: все три ракурса смотрят на РАЗНЫЕ стороны модели (overlap < 12%).
Наши данные — из вращающегося видео (все стороны видны).

**Combined GT** = объединение трёх ракурсов через visibility-weighted mean:
1. Нормализуем каждый вид: `gt_norm = gt / sum(gt[visible_verts])`
2. Weighted mean: `combined = sum(gt_norm × visible) / sum(visible)`

Итог: вершина видимая из всех 3 ракурсов → среднее трёх нормализованных значений.
Вершина видимая из 1 ракурса → только то значение.
Вершина невидимая ни откуда → 0.0.

**Покрытие (bunny):** 15 856 из 20 000 вершин имеют combined_gt > 0 (79.3%).
...
# Для пилота: добавить --models bunny A380 dragon
```

---

### 2. Текущие значения метрик для двух методов

Все цифры ниже — **после исправления back-face culling** (commit `2d79c1f`).
Тестовая модель: bunny. GT: combined (visibility-weighted mean из трёх ракурсов).
Метрики: `metrics_vs_gt_combined.{method}.metrics_covered_only`.

| Метод | CC | SIM | KLD | AUC_top10% | NSS_top10% |
|-------|-----|-----|-----|-----------|-----------|
| raycast_nearest_vertex | 0.016 | 0.335 | 10.06 | 0.476 | −0.139 |
| **cone_gaussian_on_mesh** | **0.031** | 0.495 | 1.315 | 0.472 | −0.171 |
| **screen_space_gaussian** | 0.023 | **0.523** | **0.940** | 0.452 | −0.230 |

**До исправления back-face** (для сравнения):

| Метод | CC | AUC_top10% |
|-------|-----|-----------|
| screen_space | −0.046 | 0.354 |

Улучшение от исправления: CC +0.069, AUC +0.098.

**На per-view GT (visibility_only), после фикса:**

| View | CC_visible (screen_space) | CC_visible (cone) |
|------|--------------------------|------------------|
| 300 | −0.057 | — |
| 413 | +0.159 | — |
| 599 | −0.132 | — |

*Полный 32-модельный прогон не запускался — только bunny пилот.*

---

### 3. Почему CC такая низкая?

**Это не баг. Это фундаментальный cross-condition mismatch.**

**Наши данные:** участники смотрели на вращающееся видео (17 сек, ~374° оборот).
**GT авторов:** 19 участников смотрели на СТАТИЧНОЕ изображение (7 сек, 3 фиксированных угла).

Сама статья (Section 4.5) прямо говорит:
> "fixations resulting from a dynamic scene are significantly different from those
> resulting from a static scene" (p < 0.0001 по форме распределения)

Это принципиально разные когнитивные задачи:
- **Статика:** внимание притягивают геометрически выразительные части объекта (лицо, уши, выступы).
  Центр-биас очень силён (люди смотрят в центр экрана).
- **Динамика:** внимание дополнительно управляется движением камеры, появлением скрытых частей,
  световыми эффектами при вращении.

**Что значат цифры:**

| Датасет | Метрика | CC | Условие |
|---------|---------|-----|---------|
| 3DVA combined GT | cone CC | 0.031 | ❌ Cross-condition (разные эксперименты) |
| 3DVA GT (paper, Song) | CC | ~0.47 | ✅ Same-condition (те же статичные изображения) |
| SAL3D | cone CC (bunny) | 0.769 | ✅ Same-condition (вращающееся видео) |
| Human upper bound (3DVA) | inter-observer CC | ~0.81 | ✅ Same-condition |

**Итог:** CC 0.02–0.03 на 3DVA combined GT — это не провал метода.
Это верхняя граница того, что возможно при cross-condition сравнении.
Для оценки качества метода используйте **SAL3D** (primary GT, те же условия).

**Почему cone > screen_space (0.031 vs 0.023)?**

Cone метод использует 3D ray-casting → знает ГДЕ на меше находится взгляд.
Screen_space метод не знает глубину → вершины на одних и тех же screen coords
(передняя и задняя стороны меша при разных углах вращения) смешиваются.

---

### 4. MeshMamba — не трогать

**MeshMamba полный прогон (105 моделей × 2 трека × 2 метода) уже выполнен** 2026-06-01.

Результаты лежат в:
```
results/benchmark_runs/meshmamba/
  2026-06-01_non_texture_cone/         ✅ корректны (cone)
  2026-06-01_non_texture_screen_space/ ⚠️ v1 sigma (96px, устаревший)
  2026-06-01_rgb_texture_cone/         ✅ корректны (cone)
  2026-06-01_rgb_texture_screen_space/ ⚠️ v1 sigma (96px, устаревший)
```

**Cone-результаты полностью валидны** и ничего исправлять не нужно.

Screen_space v1 (sigma=96px) имел баг — слишком широкий Гауссиан.
v2 тестировался только на Rubber_Duck (CC +0.117). Полный v2 прогон желателен,
но **не является срочным** — cone — основной метод для сравнения.

**Не запускай MeshMamba eval снова** без явной просьбы. Текущие данные достаточны.
