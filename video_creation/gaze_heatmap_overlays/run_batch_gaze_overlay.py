...
Usage:
  ...
  python ... --mode benchmark_one_turn_overlay --allow-full-batch

  # Parallel with 4 workers, skip already-rendered:
  python ... --allow-full-batch --workers 4 --skip-existing
"""
...
# Minimum file size (bytes) for --skip-existing to consider a video complete.
# Smallest completed overlay in testing was ~3.9 MB; partial kills leave <2 MB.
_MIN_OVERLAY_SIZE = 2_000_000


def _is_complete_overlay(path: Path) -> bool:
    return path.exists() and path.stat().st_size >= _MIN_OVERLAY_SIZE


def _render_worker_job(job: dict) -> dict:
    """Top-level picklable worker for ProcessPoolExecutor.

    Suppresses per-frame stdout output to avoid interleaved terminal output
    when multiple workers run simultaneously.
    """
    _null, sys.stdout = sys.stdout, io.StringIO()
    try:
        t0 = time.time()
        res = render_fixation_overlay(
            fixation_json_path=Path(job["fix_path"]),
            video_path=Path(job["video_path"]),
            output_path=Path(job["out_mp4"]),
            mode=job["mode"],
            sigma_ref_px=job["sigma_ref_px"],
            map_width=job["map_width"],
            map_height=job["map_height"],
            max_opacity=job["max_opacity"],
            drop_audio=job["drop_audio"],
        )
        return {"_ok": True, "_elapsed": time.time() - t0, **res}
...
def run_batch(
    ...
    print(f"[INFO] mode={mode}  models={len(entries)}  dry_run={dry_run}"
          f"  skip_existing={skip_existing}  workers={workers}  limit={limit}")
    for e in entries:
    ...
    # Build base row and separate renderable jobs from skips
    render_queue: list[tuple[dict, dict, dict]] = []  # (entry, row, job)
    ...
    for e in entries:
        ...
        if skip_existing and _is_complete_overlay(out_mp4):
            row["status"] = "skipped_existing"
            print(f"[SKIP_EXISTING] {model_key}")
            combined_manifest.append(row)
            continue
        ...
        render_queue.append((e, row, job))

    n_skip_existing = sum(1 for r in combined_manifest if r.get("status") == "skipped_existing")
    n_skip_no_video = sum(1 for r in combined_manifest if r.get("status") == "skipped_no_video")
    print(f"[INFO] to render: {len(render_queue)}  "
          f"skipped_existing: {n_skip_existing}  skipped_no_video: {n_skip_no_video}")

    if workers > 1 and len(render_queue) > 1:
        _run_parallel(render_queue, combined_manifest, commit, workers)
    else:
        _run_sequential(render_queue, combined_manifest, commit)

    _write_combined_manifest(output_root, combined_manifest)


def _run_sequential(
    render_queue: list[tuple[dict, dict, dict]],
    combined_manifest: list[dict],
    commit: str,
) -> None:
    for e, row, job in render_queue:
        model_key = e["model_key"]
        out_dir = Path(job["out_mp4"]).parent
        ...
        try:
            ...
            (out_dir / "manifest.json").write_text(
                json.dumps({**result, "repo_commit": commit}, indent=2)
            )
            elapsed = time.time() - t0
            print(f"  done in {elapsed:.1f}s — {result['frame_count']} frames → {job['out_mp4']}",
                  flush=True)
        except Exception as exc:
            row["status"] = "error"
            row["error"] = str(exc)
...
def _run_parallel(
    ...
    print(f"[INFO] Parallel render: {workers} workers, {len(render_queue)} jobs", flush=True)
    futures_map: dict = {}
    with ProcessPoolExecutor(max_workers=workers) as executor:
        for e, row, job in render_queue:
            fut = executor.submit(_render_worker_job, job)
            futures_map[fut] = (e, row, job)

    n_done = 0
    n_total = len(render_queue)
    for fut in as_completed(futures_map):
        e, row, job = futures_map[fut]
        model_key = e["model_key"]
        out_dir = Path(job["out_mp4"]).parent
        n_done += 1
        try:
            wr = fut.result()
            if wr["_ok"]:
                result = {k: v for k, v in wr.items() if not k.startswith("_")}
                row.update({
                    "n_json_frames": result.get("n_json_frames", ""),
                ...
                print(f"[{n_done}/{n_total}] {model_key}  {wr['_elapsed']:.1f}s"
                      f"  {result.get('frame_count','')} frames → {job['out_mp4']}", flush=True)
            else:
                row["status"] = "error"
                row["error"] = wr.get("_error", "unknown")
                print(f"[ERROR] {model_key}: {row['error']}", file=sys.stderr)
        except Exception as exc:
            row["status"] = "error"
            row["error"] = str(exc)
            print(f"[ERROR] {model_key}: {exc}", file=sys.stderr)
...
def _write_combined_manifest(output_root: Path, combined_manifest: list[dict]) -> None:
    ...
    n_skip_e = sum(1 for r in combined_manifest if r.get("status") == "skipped_existing")
    n_skip_v = sum(1 for r in combined_manifest if r.get("status") == "skipped_no_video")
    n_err = sum(1 for r in combined_manifest if r.get("status") == "error")
    print(f"\n[DONE] ok={n_ok} skipped_existing={n_skip_e} skipped_no_video={n_skip_v} errors={n_err}")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--model-list-file", type=Path, default=None,
                    help="File with one model_key per line; combined with --models if both given")
    ...
    ap.add_argument("--skip-existing", action="store_true",
                    help=f"Skip models whose overlay.mp4 already exists and is ≥{_MIN_OVERLAY_SIZE//1_000_000} MB")
...
def main() -> None:
    ...
    filter_models: list[str] | None = list(args.models) if args.models else None

    if args.model_list_file is not None:
        if not args.model_list_file.exists():
            print(f"[ERROR] --model-list-file not found: {args.model_list_file}", file=sys.stderr)
            sys.exit(1)
        file_models = [
            line.strip() for line in args.model_list_file.read_text().splitlines()
            if line.strip() and not line.startswith("#")
        ]
        if filter_models:
            filter_models = list(set(filter_models) | set(file_models))
        else:
            filter_models = file_models
