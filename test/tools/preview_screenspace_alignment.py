...
=== Outputs ===
  ...
  <output-dir>/<model>_frame<NNNN>.png   — 3-panel image per frame
  <output-dir>/<model>_summary.png       — accumulated pred vs GT
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dataset-root", type=Path,
                   default=Path(os.environ.get("MESHMAMBA_NON_TEXTURE_ROOT", "")))
    p.add_argument("--csv-root", type=Path,
                   default=Path(os.environ.get("MESHMAMBA_CSV_ROOT", "")))
    p.add_argument("--json-root", type=Path,
                   default=Path(os.environ.get("MESHMAMBA_JSON_ROOT", "")))
    return p.parse_args()
...
def main() -> None:
    ...
    gt_path   = find_gt_file(args.dataset_root / "SaliencyMap" / texture_type, args.model)
    ...
    for fr in chosen:
        ...
        out = args.output_dir / f"{args.model}_frame{fr:04d}.png"
        row.save(out)
        print(f"Saved: {out}")
    ...
    out_summary = args.output_dir / f"{args.model}_summary.png"
