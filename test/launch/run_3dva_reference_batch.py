...
def build_command(args: argparse.Namespace, task: Task) -> list[str]:
    ...
    common += ["--timing-contract", args.timing_contract]
    common += ["--delay-seconds", str(getattr(args, "delay_seconds", 0.0))]
    if getattr(args, "fixation_data_tag", None):
        common += ["--fixation-data-tag", args.fixation_data_tag]
...
def _provenance_matches(report_path: Path, args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        if current_contract == "one_turn_from_start":
            ...
            # All five fields must be present and consistent; a missing field is a mismatch.
