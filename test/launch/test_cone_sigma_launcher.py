"""Tests for the cone_sigma family launcher."""
...
launcher = _load("cone_sigma_launcher")
...
def _smoke_files(tmp_path, dataset="sal3d"):
    ...
    smoke.write_text(json.dumps({
        ...
        "point": {"dataset": dataset, "method": "cone"},
...
def test_refined_grid_and_skip_refined_policy():
    ...
    gate_edge = dict(gate, best={"sigma_deg": 2.0})
...
def test_coarse_run_creates_family_layout_and_refined_stub(tmp_path):
    ...
    result = launcher.run_cone_sigma_branch(
        manifest, results_root=tmp_path, invoke=launcher.ev.make_mock_invoke(), check_smoke=True,
    )
    coarse_root = tmp_path / "ablation" / "cone_sigma" / "sal3d" / "cone" / "coarse"
    ...
    assert result["refined_manifest_stub_path"] == (tmp_path / "ablation" / "cone_sigma" / "sal3d" / "cone" / "refined" / "refined_manifest_stub.json")
    assert result["branch_best_path"] is None


def test_coarse_skip_refined_writes_root_branch_best(tmp_path):
    manifest = _manifest(tmp_path, dataset="sal3d")
    result = launcher.run_cone_sigma_branch(
        manifest, results_root=tmp_path, invoke=_peak_invoke(1.0), check_smoke=True,
    )
    branch_root = tmp_path / "ablation" / "cone_sigma" / "sal3d" / "cone"
    assert result["promoted"] is True
...
def test_refined_run_writes_final_best_and_root_pointer(tmp_path):
    ...
    coarse_result = launcher.run_cone_sigma_branch(
...
def test_resume_reuses_identity_rows_and_does_not_duplicate_points(tmp_path):
    manifest = _manifest(tmp_path, dataset="sal3d")
    launcher.run_cone_sigma_branch(
        manifest, results_root=tmp_path, invoke=_peak_invoke(1.0), check_smoke=True,
    )
    coarse_jsonl = tmp_path / "ablation" / "cone_sigma" / "sal3d" / "cone" / "coarse" / "aggregate" / "ablation_runs.jsonl"
    before = coarse_jsonl.read_text().splitlines()
    launcher.run_cone_sigma_branch(
        manifest, results_root=tmp_path, invoke=lambda *a, **k: (_ for _ in ()).throw(AssertionError("should not rerun atoms")),
        check_smoke=True,
    )
...
def test_cone_sigma_records_model_set_signature(tmp_path):
    manifest = _manifest(tmp_path, dataset="sal3d")
    launcher.run_cone_sigma_branch(
        manifest, results_root=tmp_path, invoke=_peak_invoke(1.0), check_smoke=True,
    )
    coarse_jsonl = (
        tmp_path / "ablation" / "cone_sigma" / "sal3d" / "cone"
...
def test_real_invoke_for_manifest_uses_submission_runtime_contract(tmp_path, monkeypatch):
    ...
    invoke({"dataset": "sal3d", "method": "cone", "sigma_deg": 1.6}, "dog")

    assert captured["model"] == "dog"
    assert captured["kwargs"]["work_dir"] == (
        tmp_path / "ablation" / "_work" / "cone_sigma" / "sal3d" / "cone" / "coarse"
    )
...
def test_main_non_mock_rejects_repo_commit_drift(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(launcher, "run_cone_sigma_branch", lambda **kwargs: (_ for _ in ()).throw(
        AssertionError("run_cone_sigma_branch must not be called")
    ))
    monkeypatch.setattr(sys, "argv", [
        "cone_sigma_launcher.py", "--manifest", str(manifest_path), "--results-root", str(tmp_path),
    ])
...
def test_main_non_mock_runs_with_stage_preflight_and_real_invoke(tmp_path, monkeypatch):
    ...
    def fake_run_cone_sigma_branch(manifest, *, results_root, invoke, check_smoke):
    ...
    monkeypatch.setattr(launcher, "run_cone_sigma_branch", fake_run_cone_sigma_branch)
    monkeypatch.setattr(sys, "argv", [
        "cone_sigma_launcher.py", "--manifest", str(manifest_path), "--results-root", str(tmp_path),
    ])
