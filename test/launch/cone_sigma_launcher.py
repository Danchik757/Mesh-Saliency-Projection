"""`cone_sigma` launcher (v1) — per the Stage-2 spec.

NON-DESTRUCTIVE, LOCAL ONLY by default. Encodes the cone_sigma family from
the Stage-2 design notes:
- one branch = one (dataset, "cone") pair;
- two internal sub-stages: `coarse` (7-point grid) then `refined`-or-skip
  (5-point grid centred at the coarse best, derived deterministically).

This module does the new orchestration work (manifest validation, identity
sha1 + resume filter, promotion gates, skip-refined decision, marker writers,
refined-manifest stub) and **delegates actual evaluator execution** to the
existing `run_evaluator_sweep` machinery. It imports no baseline evaluator.

Companion spec:
`claude2_orchestra/reports/stage2_ablation/2026-06-15_cone_sigma_launcher_spec.md`.
"""
...
# ── canonical constants (spec §1.2, §5) ────────────────────────────────────────
...
COARSE_GRID: tuple[float, ...] = (0.5, 0.65, 0.8, 1.0, 1.25, 1.6, 2.0)
EDGE_EXPANSION: tuple[float, ...] = (2.5, 3.0)
REFINED_COEFS: tuple[float, ...] = (0.75, 0.875, 1.0, 1.125, 1.25)

COVERAGE_THRESHOLD = 0.70             # ≥70% non-RED
ERROR_TYPE_MAX_SHARE = 0.30           # no single error_type owns >30% of failures
NOISE_FLOOR_MULT = 0.5                # noise_floor_CC = 0.5 × std(top-3 CCs)
SKIP_REFINED_LEAD_MULT = 3.0          # best - second ≥ 3 × noise_floor


# ── manifest validation ───────────────────────────────────────────────────────

class ManifestError(ValueError):
    """Raised when a cone_sigma manifest fails validation."""
...
def validate_manifest(manifest: dict, *, check_smoke: bool = True) -> dict:
    """Validate a cone_sigma manifest. Returns the manifest unchanged on success;
    raises ManifestError with an actionable message on failure."""
    ...
    for field in ("family", "branch_stage", "dataset", "method", "texture_type",
    ...
    if sub["branch_stage"] not in ("coarse", "refined"):
        raise ManifestError(f"submission.branch_stage must be coarse|refined")
    ...
    if sweep.get("radius_sigma_mult") != RADIUS_SIGMA_MULT:
        raise ManifestError(f"sweep.radius_sigma_mult must be {RADIUS_SIGMA_MULT}")
    sigma_values = sweep.get("sigma_values")
    if not (isinstance(sigma_values, list) and sigma_values and all(isinstance(v, (int, float)) for v in sigma_values)):
    ...
    if sub["branch_stage"] == "coarse":
        ...
        if sorted(sigma_values) != expected:
            raise ManifestError(
                f"coarse sweep.sigma_values must equal {expected}, got {sorted(sigma_values)}")
        if consumes:
        ...
        for f in ("family", "stage", "dataset", "method", "repo_commit", "best_params"):
            if f not in cb:
                raise ManifestError(f"coarse_best marker missing field: {f}")
        ...
        if sorted(sigma_values) != expected_refined:
            raise ManifestError(
                f"refined sweep.sigma_values must equal {expected_refined}, got {sorted(sigma_values)}")
    ...
    if check_smoke:
        smoke_path = Path(sub["smoke_artifact"])
        smoke = _validate_smoke(manifest, smoke_path)
    return manifest
...
def identity_sha1(**fields) -> str:
    """Deterministic per-atom resume key. Uses the 13-field tuple from the
    Stage-2 server contract; pickling-free (string concat + sha1)."""
...
def refined_grid(sigma_star: float, *, coefs: tuple[float, ...] = REFINED_COEFS) -> list[float]:
    """Deterministic 5-point refined grid around `sigma_star`. Spec §5.3.
    Rounded to 2 dp; duplicates collapsed; sorted ascending."""
    return sorted({round(c * float(sigma_star), 2) for c in coefs})


# ── promotion gates (spec §5.1 / §5.3) ────────────────────────────────────────

def _row_status(row: dict) -> str:
    """Map an aggregate row to RED / AMBER / GREEN per spec §6.3."""
...
def evaluate_stage(rows: list[dict], expected_points: int) -> dict:
    """Apply the three promotion gates plus best-region identifiability.
    Returns a structured result dict with `promoted`, `failed_gates`,
    `gate_details`, and (if promoted) ranked best/second/third rows + noise floor.
    """
    if expected_points <= 0:
        raise ValueError("expected_points must be > 0")
    statuses = {r.get("sigma_deg"): _row_status(r) for r in rows}
    ...
    # Best-region identifiability: rank non-RED rows by CC desc.
...
def decide_skip_refined(gate_result: dict, *, grid: tuple[float, ...] = COARSE_GRID,
    ...
    """Spec §5.2. Skip iff interior best AND lead ≥ 3 × noise_floor AND not forced."""
    if force_refined or not gate_result.get("promoted"):
        return False
    best = gate_result["best"]
    grid_sorted = sorted(grid)
    sigma_star = float(best.get("sigma_deg"))
...
# ── marker writers (atomic via temp + rename) ─────────────────────────────────
...
def _stage_marker_payload(stage: str, manifest: dict, gate_result: dict, *,
    ...
    sigma_star = float(best["sigma_deg"])
    return {
        "family": FAMILY,
        "stage": stage,
        "dataset": sub["dataset"],
        ...
        "best_params": {SIGMA_PARAM: float(best["sigma_deg"]), "radius_sigma_mult": RADIUS_SIGMA_MULT},
        ...
        "is_interior_best": bool(stage == "coarse" and sigma_star in coarse_grid_values and
                                 0 < coarse_grid_values.index(sigma_star) < len(coarse_grid_values) - 1)
...
def write_branch_best(branch_dir: Path, *, stage: str, points_to: str, source: dict) -> Path:
    """Single authoritative pointer at the family-branch root."""
    payload = {
        "family": FAMILY,
...
# ── refined manifest stub ─────────────────────────────────────────────────────

def emit_refined_manifest_stub(coarse_manifest: dict, coarse_best_marker_path: Path,
                               out_path: Path) -> Path:
    """v1 does not auto-submit refined; it emits a stub for human approval."""
    ...
    stub = {
        "submission": sub,
        "sweep": {"sigma_param": SIGMA_PARAM, "radius_sigma_mult": RADIUS_SIGMA_MULT,
                  "sigma_values": refined_grid(sigma_star), "edge_expansion": False},
...
def load_resume_ok_rows(stage_dir: Path) -> dict[str, dict]:
    """Load the latest reusable per-model ok rows keyed by identity_sha1."""
...
def _stage_dirs(results_root: Path, manifest: dict) -> tuple[Path, Path, Path]:
    """Return (family_branch_dir, current_stage_dir, refined_stage_dir)."""
    sub = manifest["submission"]
...
def _execute_stage(manifest: dict, *, results_root: Path, stage_dir: Path, invoke,
    ...
    """Execute one (point, model) per atom, recording into the existing layer.
    Wraps `invoke` so each per-model row gets its identity_sha1 stamped, and
    so already-ok atoms are returned as `status="skipped_resume"`."""
    sub = manifest["submission"]
    sigma_values = list(manifest["sweep"]["sigma_values"])
    ...
    for v in sigma_values:
        point = ev.make_sigma_grid(
            sub["dataset"], METHOD, sigma_param=SIGMA_PARAM, sigma_values=[v],
    ...
    def _identity_for(point: dict, model: str) -> str:
        return identity_sha1(
            ...
            sigma_value=float(point["sigma_deg"]),
            radius_sigma_mult=RADIUS_SIGMA_MULT,
    ...
    def _make_params(point: dict) -> dict:
        return {
            ...
            "command": "cone_sigma_launcher.run_cone_sigma_branch",
            "notes": f"family={FAMILY} stage={sub['branch_stage']}",
...
def run_cone_sigma_branch(manifest: dict, *, results_root: Path, invoke,
                          check_smoke: bool = True) -> dict:
    """Top-level orchestration for one (branch, stage) submission.

    Returns a dict summarising the outcome:
      {
        "stage": "coarse"|"refined",
        "promoted": bool,
        "skip_refined": bool|None,
        "coarse_best_path": Path|None,
        "final_best_path": Path|None,
        "branch_best_path": Path|None,
        "held_path": Path|None,
        "refined_manifest_stub_path": Path|None,
      }
    """
    ...
    _execute_stage(manifest, results_root=results_root, stage_dir=stage_dir,
    ...
    out: dict = {"stage": sub["branch_stage"], "promoted": gate["promoted"],
    ...
    if not gate["promoted"]:
        out["held_path"] = write_held(stage_dir, manifest=manifest,
...
# ── CLI (v1: --validate-only and --mock) ──────────────────────────────────────

def main() -> int:
    ap = argparse.ArgumentParser(description="cone_sigma launcher (v1).")
    ...
    try:
        ...
        print(f"[cone_sigma] MANIFEST INVALID: {exc}", file=sys.stderr)
        return 2

    if args.validate_only:
        print("[cone_sigma] manifest OK")
        return 0
    if args.mock:
        ...
        try:
            ...
            print(f"[cone_sigma] RUNTIME GATE FAILED: {exc}", file=sys.stderr)
            return 2
        invoke = _real_invoke_for_manifest(manifest, results_root=args.results_root)
    result = run_cone_sigma_branch(
    ...
    print(f"[cone_sigma] {verdict} stage={result['stage']} "
