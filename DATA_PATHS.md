...
| Type | Path |
|------|------|
| OBJ files | `/Users/admin/Documents/LAB/SALIENCY_code/GAZE_DATA/datasets/SAL3D/SAL3D_Dataset/Meshes/` |
| Raw gaze data (per-model) | `/Users/admin/Documents/LAB/SALIENCY_code/GAZE_DATA/datasets/SAL3D/SAL3D_Dataset/Gaze/` |
| Smooth gaze neighbors (auxiliary, not GT) | `/Users/admin/Documents/LAB/SALIENCY_code/GAZE_DATA/datasets/SAL3D/SAL3D_Dataset/Smooth_Gaze/` |
| Camera JSON (per-model) | `/Users/admin/Documents/LAB/SALIENCY_code/GAZE_DATA/jsons_for_models/SAL3D_json/` (to be generated) |
| Video MP4 | `/Users/admin/Documents/LAB/SALIENCY_code/videos/SAL3D/` |

File naming patterns (SAL3D):
- OBJ: `<model>.obj` (flat, no subfolders)
- JSON: `Sal3D_<model>.json` (generated by render script or locally)
- Gaze: `<model>.txt` (raw gaze, 8 columns per line)
- Smooth Gaze: `<model>_neighbors.txt` (neighbor lists for smoothing, not direct GT maps)
- MP4: `SAL3D_<model>.mp4`

Coverage (57 OBJ / 58 Gaze / 53 Smooth Gaze):
- Missing Smooth Gaze (6): MaxPlanck, dog, flowerpot, gamecontroller, prot, spanner
- Missing Gaze (2): gamecontroller, spanner
- Gaze without OBJ (3): AudiRS5, bimba, blade (can be ignored)

Render script settings (from `sal_render_1.py`):
- `forward_axis='Z', up_axis='Y'` (DIFFERENT from 3DVA's `'X'/'Z'`!)
- FOV = 60°, camera at world `(0, -1.5, bbox_height/2 + 0.5)`
- Object rotates 360° over 22 seconds, start angle deterministic (SHA256 seed from model name)
...
SSH alias: `vg-intellect`, user `29d_kon`
...
REPROJECT_VIDEO_SAL3D_ROOT        # → .../SALIENCY_code/videos/SAL3D
